...
PWM 基地址：`0x1001_5000`。全局寄存器位于偏移 `0x00`；每通道占用 `0x10` 字节（4 个寄存器），通道 N 基偏移为 `0x10 + N * 0x10`。

**全局寄存器**：

| Offset | 寄存器 | 访问 | 复位值 | 描述 |
| --- | --- | --- | --- | --- |
| `0x00` | `PWM_GLB` | R/W | `0x0000_0000` | 全局控制/状态寄存器 |

**通道寄存器**（通道 N，偏移基址 = `0x10 + N * 0x10`）：
...
#### 8.3.1 `PWM_CHn_CTRL`（Offset `0x10 + N*0x10 + 0x00`）— Channel Control
...
#### 8.3.2 `PWM_CHn_PERIOD`（Offset `0x10 + N*0x10 + 0x04`）— Period Value

| Bit | 名称 | 访问 | 复位 | 描述 |
| --- | --- | --- | --- | --- |
| `31:0` | `PERIOD` | R/W | `0` | 周期值：计数器从 0 计数到 PERIOD 后归零重载 |

#### 8.3.3 `PWM_CHn_DUTY`（Offset `0x10 + N*0x10 + 0x08`）— Duty Cycle Value
...
#### 8.3.4 `PWM_CHn_CNT`（Offset `0x10 + N*0x10 + 0x0C`）— Current Counter

| Bit | 名称 | 访问 | 复位 | 描述 |
| --- | --- | --- | --- | --- |
| `31:0` | `CNT` | R | `0` | 当前计数值（只读） |

#### 8.3.5 `PWM_GLB`（Offset `0x00`）— Global Control/Status
